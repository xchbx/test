...
void bs_init( bs_t *s, void *p_data, int i_data )
{
    ...
    s->p       = (byte*)p_data;
    s->p_end   = (byte*)s->p + i_data;
    s->i_left  = 8;
}
int bs_pos( bs_t *s )
{
    return static_cast<int>( 8 * ( s->p - s->p_start ) + 8 - s->i_left );
}
int bs_eof( bs_t *s )
{
...
int bs_show( bs_t *s, int i_count )
{
    if( s->p < s->p_end && i_count > 0 )
    {
        int i_cache = ((s->p[0] << 24)+(s->p[1] << 16)+(s->p[2] << 8)+s->p[3]) << (8-s->i_left);
...
void bs_write( bs_t *s, int i_count, int i_bits )
{
    if( s->p >= s->p_end - 4 )
        return;
    while( i_count > 0 )
...
void bs_write1( bs_t *s, int i_bit )
{
    if( s->p < s->p_end )
    {
        *s->p <<= 1;
