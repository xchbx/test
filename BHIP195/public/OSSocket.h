...
public:
	int Open(bool resue = false);
	int Open(const char* local_ip, int local_port);

	void Close();

	int SendTo(const char * local_ip, const void* buf, int len, const OS_SockAddr&  peers);
...
{
public:
	int Open(const char* mcast_ip, int port, const char* local_ip, const char *local_ip_name = NULL);
	void Close();
