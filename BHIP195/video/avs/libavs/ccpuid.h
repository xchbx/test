...
INLINE int	simd_sse_level(int* phwsse)
{
	...
	#ifdef CCPUID_X86
		const uint32_t	BIT_D_SSE = (1 << 25);	// bit 25
		const uint32_t	BIT_D_SSE2 = (1 << 26);	// bit 26
		const uint32_t	BIT_C_SSE3 = (1 << 0);	// bit 0
		const uint32_t	BIT_C_SSSE3 = (1 << 9);	// bit 9
		const uint32_t	BIT_C_SSE41 = (1 << 19);	// bit 19
		const uint32_t	BIT_C_SSE42 = (1 << 20);	// bit 20
		uint32_t dwBuf[4];
